...
Los ensayos se realizaron en el \textbf{Canal de Experiencias de Arquitectura Naval (CEAN)}. Esta instalación cuenta con un vaso de hormigón rectilíneo con las siguientes dimensiones principales:
...
El canal está equipado con un carro dinamométrico capaz de alcanzar una velocidad máxima de \textbf{4.0 m/s}, lo cual cubre holgadamente el rango de velocidades de Froude requeridas para los modelos analizados en este estudio.
...
\subsection{Estudio de Convergencia de Malla (GCI)} \label{sec:convergencia_de_malla}
...
Los resultados detallados de este estudio, incluyendo los valores de $R$, $p$ y el $GCI$ resultante para cada velocidad, se presentan al inicio del Capítulo 4. Basado en dicho análisis, se seleccionó la \textbf{malla fina} para todas las simulaciones de producción presentadas en esta tesis.

\subsubsection{Resultado de convergencia de malla}

REESCRIBIR ESTA SECCIÓN

Antes de evaluar el comportamiento físico del factor de forma, es imperativo verificar la fiabilidad de la solución numérica. La Tabla \ref{tab:CFD_TPA_1_20} resume los indicadores de calidad de malla para las simulaciones del \textit{P1} a escala 1:20.

Se observa que la estrategia de mallado garantiza un valor de $y^+$ promedio cercano a 1 (resolución de la subcapa viscosa) para todo el rango de velocidades. Asimismo, el GCI para la malla fina se mantiene por debajo del 0.2\% en el rango operativo de interés ($F_n \leq 0.36$). Para las velocidades superiores ($F_n \geq 0.36$), el GCI aumenta notablemente debido a la mayor complejidad del campo de flujo en ese régimen; no obstante, estas condiciones quedan fuera del rango operativo relevante del buque. En conjunto, estos indicadores confirman que los resultados presentados a continuación son independientes de la discretización espacial y que las variaciones observadas responden a fenómenos físicos y no tienen origen numérico.

\begin{table}[H]
\centering
\caption{Verificación numérica para el \textit{P1} a escala 1:20 (Malla Fina) en Calado 3.}
...
La Tabla~\ref{tab:CFD_KCS} presenta la verificación numérica para el modelo 
KCS a escala 1:79. Los valores de $y^+$ y GCI garantizan una solución 
numéricamente robusta en todo el rango operativo.
