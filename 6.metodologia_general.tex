...
El tercer pesquero analizado fue diseñado y construido por el Astillero Naval Federico Contessi y Cía. S.A.
...
Los valores cuantitativos finales de la incertidumbre combinada y expandida para las distintas velocidades de ensayo se presentan en detalle junto con los resultados experimentales en el Capítulo~\ref{cap:resistencia_factor_de_forma}.
...
%
\begin{equation}
    C_{TM} = (1 + k)\,C_{FM0} + C_W
\end{equation}
%
donde $C_{FM0}$ es el coeficiente de fricción del modelo calculado mediante
la línea ITTC-57 (Sección~\ref{sec:ITTC57}) al número de Reynolds del
modelo, $(1+k)$ es el factor de forma determinado según los métodos
descritos en el Capítulo~\ref{cap:resistencia_factor_de_forma}, y $C_W$
es el coeficiente de resistencia por formación de olas, obtenido como
residuo:
%
\begin{equation}
    C_W = C_{TM} - (1+k)\,C_{FM0}
\end{equation}

La hipótesis central del método es que $C_W$ es independiente del número
de Reynolds y escala directamente con el número de Froude. Bajo este
supuesto, la resistencia total del buque a escala buque se estima como:
%
...
la línea ITTC-57 al número de Reynolds del buque, $k_S$ es el factor de
forma a escala buque, $\Delta C_F$ es la corrección por rugosidad de la
superficie y $C_A$ es el coeficiente de correlación modelo--buque, ambos
recomendados por la ITTC \citep{ITTC_Performance_prediction}.

El método estándar asume $k_S = k_M$, es decir, que el factor de forma es
independiente de la escala. Como se analiza en profundidad en el
Capítulo~\ref{cap:resistencia_factor_de_forma}, esta hipótesis no se
cumple para los pesqueros estudiados en esta tesis.
...
%
\begin{equation}
    (1+k) = \frac{\overline{C}_{T,\,\text{DB}}}{C_{F0}}
\end{equation}
%
donde $\overline{C}_{T,\,\text{DB}}$ es el coeficiente de resistencia total
medio obtenido de la simulación de doble cuerpo y $C_{F0}$ es el coeficiente
de la línea ITTC-57, definido en la Sección~\ref{sec:ITTC57}.
...
Los resultados detallados de este estudio, incluyendo los valores de $R$, $p$ y el $GCI$ resultante para cada velocidad, se presentan al inicio del Capítulo~\ref{cap:resistencia_factor_de_forma}. Basado en dicho análisis, se seleccionó la \textbf{malla fina} para todas las simulaciones de producción presentadas en esta tesis.

\subsubsection{Resultado de convergencia de malla}

% TODO: REESCRIBIR ESTA SECCIÓN
