...
prestaciones. Las definiciones adimensionales de base ($K_T$, $K_Q$, $\eta_0$ y el
número de Reynolds de hélice $Re_{co}$ a $r/R=0.75$) se introdujeron en el
Capítulo~\ref{chap:antecedentes} (Sección~\ref{sec:pow_antec}), donde el número de
Reynolds de hélice se define en la Ecuación~\eqref{eq:Re_helice}. El procedimiento estándar recomendado por la ITTC para este
escalado es el método ITTC-78 \citep{ITTC_Performance_prediction}, que constituye el
gemelo propulsivo de la extrapolación de casco descrita más arriba y la referencia
frente a la cual se contrastan los resultados del Capítulo~\ref{chap:helices}.

El método parte de una hipótesis central: el efecto de escala sobre el propulsor es de
naturaleza \emph{puramente friccional}. Bajo este supuesto, la corrección se construye
a partir de la variación del coeficiente de arrastre bidimensional de la sección de
pala representativa, tomada a $r/R = 0.75$, entre las condiciones de modelo y de buque:
%
\begin{equation}
...
\end{equation}
%
donde los coeficientes de arrastre de modelo y de buque se evalúan, respectivamente,
mediante
%
\begin{align}
...
\end{align}
%
En estas expresiones $c$ y $t$ son la cuerda y el espesor máximo de la sección a
$r/R = 0.75$, $Re_{co}$ es el número de Reynolds local de esa sección definido en la
Ecuación~\eqref{eq:Re_helice}, y $k_p$ es la rugosidad equivalente de la pala, para la
que se adopta el valor estándar $k_p = 30\times10^{-6}$~m recomendado por la ITTC en
ausencia de mediciones directas. El coeficiente $C_{D,M}$ responde a una ley de
fricción dependiente del Reynolds, mientras que $C_{D,S}$ corresponde al régimen
rugoso plenamente desarrollado propio de la escala buque.

La diferencia $\Delta C_D$ se traslada a los coeficientes de empuje y de par mediante
las correcciones
%
\begin{equation}
...
\end{equation}
%
donde $P/D$ es la relación de paso a $r/R = 0.75$, $Z$ el número de palas y $D$ el
diámetro. Las características corregidas a escala buque resultan finalmente
%
\begin{equation}
    K_{TS} = K_{TM} - \Delta K_T,
    \qquad
    K_{QS} = K_{QM} - \Delta K_Q,
    \qquad
