...
\begin{figure}[htpb]
    ...
    \caption{Resistencia total al avance (EFD) en función de $Fr$ para el Calado 1 (0.165\,m).}
...
\begin{figure}[htpb]
    ...
    \caption{Resistencia total al avance (EFD) en función de $Fr$ para el Calado 2 (0.180\,m).}
...
\begin{figure}[htpb]
    ...
    \caption{Resistencia total al avance (EFD) en función de $Fr$ para el Calado 3 (0.195\,m).}
...
ponderada por la incertidumbre de la coordenada vertical $C_{TM}/C_{F0}$.
...
Conviene precisar que esta incertidumbre corresponde a la componente
\emph{estadística} del ajuste (la propagación de los errores aleatorios de
medición a través de la regresión) y resulta sensiblemente menor que la
incertidumbre relativa de cada punto individual ($\sim 4\%$). Esta reducción
no es anómala: la ordenada al origen es un parámetro determinado por el
conjunto de las mediciones, de modo que su incertidumbre disminuye con el
número de puntos y con su grado de alineación, de forma análoga a como la
incertidumbre de un promedio decrece respecto de la de cada medición. Cabe
señalar que este valor no incorpora eventuales errores sistemáticos, comunes
a todas las corridas, que no se reducen por este mecanismo.
...
$8$ y el $11\%$. La diferencia es coherente con el comportamiento de ambos
cascos frente al método de Prohaska: mientras que en el \textit{P1} los
puntos experimentales se apartan de la recta teóric, y son además mucho
menos numerosos en el rango de ajuste, en el \textit{KCS} se alinean estrechamente sobre ella,
lo que se traduce en una determinación mucho más precisa de $(1+k)$. Esta
...
posición de la primera celda. No obstante, la región "buffer"
...
superponen, para cada calado, el valor adoptado por Prohaska (línea horizontal 
...
\begin{figure}[htpb]
    ...
    \caption{Comparación entre el valor de $(1+k)$ adoptado por Prohaska 
...
análisis a un rango adecuado de números de Froude, la dispersión inicial del 
método de Prohaska se reduce de manera apreciable en ambos enfoques. La 
adopción de un intervalo acotado y físicamente consistente confirma que las 
desviaciones observadas fuera de ese rango no responden a errores sistemáticos 
de medición o modelado, sino a la violación de los supuestos del método de 
Prohaska. Aun así, la concordancia entre los valores adoptados por cada enfoque 
no es uniforme: resulta estrecha en el Calado~1, pero en los Calados~2 y~3 el 
valor numérico se sitúa por encima del experimental. Esta discrepancia residual 
es consistente con la curvatura de signo opuesto que presentan los diagramas de 
Prohaska experimental y numérico, y pone de manifiesto que el ajuste conserva 
una sensibilidad no despreciable al rango escogido y al tratamiento de la 
transición, aun dentro del intervalo considerado óptimo.
...
K_M$ en función de $\lambda$) a varuas escaladas calculadas para el \textit{P1} (1:20, 1:18, 1:16, 1:14, 1:12, 1:10, 1:7.5 1:5, 1:2.5 y 1:1), la pendiente resultante es de
...
"transom form factor" (factor de formqa de popa espejo) $k_{tr}>0$ \citep{korkmaz_wetted_transom}. Su
...
flotación, a medio calado sumergido y cerca del fondo. La coordenada 
...
(Ecuación~\eqref{eq:CTS3_menos_CTS1}), precisamente porque $k_S \gg k_M$ 
