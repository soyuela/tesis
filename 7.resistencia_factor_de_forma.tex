...
Asimismo aunque esta sección se centra inicialmente en el modelo objetivo (\textit{P1}), el análisis experimental incluye también una campaña independiente con el modelo del portacontenedores \textit{KCS}. La incorporación de este buque de referencia obedece al interés de complementar el análisis mediante la inclusión de una geometría radicalmente distinta a los pesqueros: una esbeltez mucho mayor ($L/B \approx 7{.}1$ vs. 2.8–3.5), una popa afinada con transición suave al codaste en contraste con la popa robusta de los pesqueros, y la ausencia de dormido significativo frente al dormido pronunciado de los buques pesqueros. Esta comparación permite examinar cómo la hidrodinámica del factor de forma varía cuando se introducen cambios fundamentales en las proporciones globales y la configuración local del casco.
...
A partir de los resultados experimentales obtenidos para las distintas velocidades y condiciones de calado, se construyeron los diagramas del método de Prohaska (presentado en la Sección \ref{sec:prohaska_antec}), graficando $\frac{C_T}{C_F}$ frente a $\frac{Fr^4}{C_F}$ (Figura~\ref{fig:prohaska_tpa}).  
...
Sin embargo, en los tres calados se observa una ligera curvatura en los puntos experimentales, especialmente a bajos números de Froude, lo que indica una desviación del comportamiento lineal esperado. En esta y en todas las figuras siguientes que presentan diagramas de Prohaska o resultados derivados de este método, las líneas verticales punteadas en $Fr = 0{.}10$ y $Fr = 0{.}20$ delimitan el rango de Froude recomendado por la ITTC para la aplicación del método (Sección~\ref{sec:prohaska_antec}), dentro del cual se asume que la resistencia por olas es despreciable y es válida la extrapolación lineal utilizada para obtener $(1+k)$.

\begin{figure}[htpb]
\centering
\includegraphics[width=0.6\textwidth]{images/prohaska_tpa_comparacion.pdf}
\caption{Diagramas del método de Prohaska para los tres calados del \textit{P1}. La desviación respecto de la linealidad teórica se acentúa a bajos números de Froude. Las líneas verticales punteadas indican $Fr = 0{.}10$ y $Fr = 0{.}20$ para el \textit{P1}.}
\label{fig:prohaska_tpa}
\end{figure}

Una primera explicación de esta curvatura se relaciona con fenómenos de ``laminarización'' parcial del flujo a bajas velocidades, como se describe 
...
por la línea de correlación ITTC-57 o por la aproximación ${\sim}Fr^4$ para 
...
Finalmente para cuantificar la incertidumbre asociada al ajuste lineal
de Prohaska, se aplicó el método propuesto en \citet{york}. A diferencia
de una regresión por mínimos cuadrados ordinaria, que asume la variable
independiente libre de error, el método de York resuelve el ajuste lineal
ponderando cada punto por la incertidumbre de sus dos coordenadas y
proporciona, además de la pendiente y la ordenada al origen, la
incertidumbre de esta última, que en el diagrama de Prohaska corresponde
directamente a $(1+k)$.

En este caso, la coordenada horizontal $Fr^4/C_{F0}$ depende esencialmente
de la velocidad del carro, cuya incertidumbre relativa ($0{.}07\%$, según la
Tabla~\ref{tab:uncert_tpa}) es dos órdenes de magnitud menor que la de la
resistencia. Por este motivo se consideró despreciable el error en el eje
horizontal, con lo que el método de York se reduce a una regresión lineal
ponderada por la incertidumbre de la coordenada vertical $C_{T}/C_{F0}$.
La incertidumbre de cada punto se obtuvo propagando la incertidumbre
combinada de la resistencia $U_D$ (media de cuatro repeticiones,
$5{.}35\%$, $2{.}04\%$ y $0{.}83\%$ para $Fr = 0{.}14$, $0{.}26$ y $0{.}37$
respectivamente, interpolada linealmente en $Fr$) a la variable
$C_{TM}/C_{F0}$, que es lineal en la resistencia medida.

La Figura~\ref{fig:york_tpa} ilustra el ajuste ponderado obtenido para los tres calados, y la Tabla~\ref{tab:york_tpa} reúne los valores de $(1+k)$ y su
incertidumbre expandida. Los factores de forma
resultantes coinciden, dentro de la milésima, con los obtenidos por
regresión simple (Tabla~\ref{tab:form_factors}), lo que confirma que la
ponderación no introduce sesgos apreciables en el valor central. Las
incertidumbres asociadas, del orden de $\pm 0{.}10$ a $\pm 0{.}13$ (equivalentes
a un $8$--$11\%$ de $(1+k)$), reflejan
la fuerte sensibilidad del ajuste de Prohaska a la incertidumbre
experimental de la resistencia en el reducido rango de velocidades
empleado y son coherentes con la dispersión observada en los diagramas.
...
\begin{figure}[htpb]
     ...
     \begin{subfigure}[b]{0.6\textwidth}
         \centering
         \includegraphics[width=\textwidth]{images/york_tpa_165.pdf}
         \caption{Calado T1.}
         \label{fig:york_165}
     \end{subfigure}
     \vspace{0.5cm}
     \begin{subfigure}[b]{0.6\textwidth}
         \centering
         \includegraphics[width=\textwidth]{images/york_tpa_180.pdf}
         \caption{Calado T2.}
         \label{fig:york_180}
     \end{subfigure}
     ...
     \begin{subfigure}[b]{0.6\textwidth}
         \centering
         \includegraphics[width=\textwidth]{images/york_tpa_195.pdf}
         \caption{Calado T3.}
         \label{fig:york_195}
     \end{subfigure}
     
     \caption{Ajuste lineal con ponderación mediante el método de York aplicado al \textit{P1} para la estimación de $(1+k)$ y su incertidumbre. Las líneas verticales punteadas indican $Fr = 0{.}10$ y $Fr = 0{.}20$ para el \textit{P1}.}
     \label{fig:york_tpa}
\end{figure}
...
\begin{table}[H]
\centering
\caption{Factor de forma $(1+k)$ del \textit{P1} e incertidumbre expandida
obtenidos mediante el método de York (error despreciable en el eje
horizontal) para los tres calados.}
\begin{tabular}{lccc}
\hline
 & \textbf{T1: 0.165 m} & \textbf{T2: 0.180 m} & \textbf{T3: 0.195 m} \\
\hline
...
correlación ITTC-57, aproximación ${\sim}Fr^4$) y otras son intrínsecas 
...
se realizó una segunda campaña experimental utilizando el modelo \textit{KCS} 
...
grafica $C_{T}/C_{F0}$ frente a $Fr^4/C_{F0}$, siendo $C_{F0}$ el 
...
es consistente con lo esperable para un casco como el \textit{KCS}: la hipótesis de 
...
adherido y predominantemente turbulento. El \textit{KCS} opera en un rango de Reynolds 
...
\caption{Diagrama de Prohaska para el modelo \textit{KCS}. Se grafica 
...
el valor de $1+k$. Las líneas verticales punteadas indican $Fr = 0{.}10$ y $Fr = 0{.}20$ para el \textit{KCS}.}
...
adoptado fue $(1+k) = 1{.}052$, correspondiente al rango $0{.}13 < Fr < 0{.}20$.

Sobre este mismo rango se aplicó el método de York descrito para el
\textit{P1}, con el fin de cuantificar la incertidumbre del factor de forma
del \textit{KCS}. Como cada punto del diagrama corresponde al promedio de varias
corridas repetidas, se adoptó para la coordenada vertical una incertidumbre
proporcional a la resistencia con un coeficiente de variación constante,
igual al valor mediano observado en las repeticiones ($\sim 4\%$). Este
criterio evita que algún grupo con pocas corridas casualmente coincidentes
reciba un peso desproporcionado en la regresión ponderada. El ajuste
confirma el valor adoptado y le asocia una incertidumbre expandida de
$\pm 0{.}024$, esto es, una incertidumbre relativa del $\pm 2{.}3\%$, como se
muestra en la Figura~\ref{fig:york_kcs}.

\begin{figure}[htpb]
\centering
\includegraphics[width=0.6\textwidth]{images/york_kcs.pdf}
\caption{Ajuste lineal con ponderación mediante el método de York aplicado al \textit{KCS} para la estimación de $(1+k)$ y su incertidumbre. Las líneas verticales punteadas indican $Fr = 0{.}10$ y $Fr = 0{.}20$ para el \textit{KCS}.}
\label{fig:york_kcs}
\end{figure}

Conviene precisar que esta incertidumbre corresponde a la componente \emph{estadística} del ajuste (la propagación de los errores aleatorios de medición a través de la regresión) y resulta sensiblemente menor que la incertidumbre relativa de cada punto individual ($\sim 4\%$). Esta reducción no es anómala: aunque $(1+k)$ se obtiene por extrapolación a la abscisa nula, y la incertidumbre de una ordenada al origen crece con la distancia entre el centroide de los datos y el origen, en el \textit{KCS} los puntos son numerosos, se alinean estrechamente sobre la recta y su rango de ajuste se aproxima al origen, de modo que el término de palanca resulta pequeño y predomina el efecto del número de mediciones. Cabe señalar que este valor no incorpora eventuales errores sistemáticos, comunes a todas las corridas, que no se reducen por este mecanismo.

Este valor contrasta de manera marcada con el obtenido para el \textit{P1}
(Tabla~\ref{tab:york_tpa}), cuya incertidumbre relativa se situaba entre el
$8$ y el $11\%$, superior incluso a la de los puntos experimentales que la
originan. La diferencia responde al mismo mecanismo: en el \textit{P1} los
puntos experimentales se apartan de la recta teórica, son mucho menos
numerosos en el rango de ajuste y su centroide queda más alejado del origen,
con lo que el término de palanca de la extrapolación domina y amplifica la
incertidumbre de la ordenada en lugar de reducirla; en el \textit{KCS}, en
cambio, los puntos se alinean estrechamente sobre la recta y el rango de
ajuste se aproxima al origen, lo que se traduce en una determinación mucho
más precisa de $(1+k)$. Esta
comparación cuantitativa respalda la observación cualitativa de que el \textit{KCS}
satisface los supuestos del método de Prohaska con mayor fidelidad que el
\textit{P1}, y refuerza la conveniencia de recurrir al método de doble cuerpo
para el \textit{P1}, donde el ajuste experimental resulta intrínsecamente
menos confiable.

\begin{figure}[htpb]
\centering
\includegraphics[width=0.6\textwidth]{images/efd_form_factor_kcs_range.pdf}
\caption{Variación del factor de forma $(1+k)$ para el modelo \textit{KCS} en función 
...
el LabHiNO con los recopilados en \citet{TerzievDataSet} a partir de ensayos 
...
\caption{Comparación del factor de forma $(1+k)$ del \textit{KCS} obtenido en el 
LabHiNO (punto rojo) con resultados experimentales históricos del \textit{KCS} 
compilados de literatura internacional en \citet{TerzievDataSet} 
...
utilizando el solver multifásico \texttt{interFoam} para capturar la 
...
$R_{T}$, el enfoque numérico permite separar las contribuciones de las fuerzas 
...
En las Figuras~\ref{fig:cfd_165_tpa}, \ref{fig:cfd_180_tpa} y~\ref{fig:cfd_195_tpa} se presentan 
...
\begin{figure}[htpb]
    \centering
    \includegraphics[width=0.6\textwidth]{images/cfd_165.pdf}
    \caption{Resistencia total al avance del \textit{P1} obtenida por CFD en función de $Fr$ para el Calado 1 (0.165\,m).}
    \label{fig:cfd_165_tpa}
\end{figure}

\begin{figure}[htpb]
    \centering
    \includegraphics[width=0.6\textwidth]{images/cfd_180.pdf}
    \caption{Resistencia total al avance del \textit{P1} obtenida por CFD en función de $Fr$ para el Calado 2 (0.180\,m).}
    \label{fig:cfd_180_tpa}
\end{figure}

\begin{figure}[htpb]
    \centering
    \includegraphics[width=0.6\textwidth]{images/cfd_195.pdf}
    \caption{Resistencia total al avance del \textit{P1} obtenida por CFD en función de $Fr$ para el Calado 3 (0.195\,m).}
...
(regresión lineal sobre el rango $0{.}16 < Fr < 0{.}20$, donde la curva
$C_{T}/C_{F0}$ vs $Fr^4/C_{F0}$ se aproxima mejor a la linealidad teórica)
se obtienen los valores de factor de forma numérico $(1+k)$ resumidos en la
Tabla~\ref{tab:form_factors_cfd} para los tres calados. A diferencia del caso
experimental, donde el factor de forma decrece monótonamente con el calado, los
valores numéricos no siguen una tendencia uniforme, lo que refleja la mayor
sensibilidad del ajuste de Prohaska en el entorno numérico a la curvatura
introducida por el tratamiento de la transición en el modelo $k$--$\omega$ SST.

\begin{table}[H]
\centering
\caption{Factor de forma $k$ para los tres calados del \textit{P1} obtenido
por el método de Prohaska aplicado a las simulaciones CFD bifásicas.}
\begin{tabular}{lccc}
\hline
\textbf{Método} & \textbf{T1: 0.165 m} & \textbf{T2: 0.180 m} & \textbf{T3: 0.195 m} \\
\hline
...
y la curvatura observada en el diagrama $C_{T}/C_{F0}$ vs $Fr^4/C_{F0}$ a 
bajas velocidades, se optó por una estrategia alternativa: el método de 
``doble cuerpo'', descripto en la Sección~\ref{sec:doble_cuerpo}.
...
\multicolumn{5}{c}{\rule{\dimexpr\textwidth-2\tabcolsep}{0.2pt}} \\[0.4em]
\textbf{0.100} & 0.40 & 8.42 & 12.0 & 108 \\
...
la complejidad del campo de flujo a alta velocidad en esa escala. Aunque se lo incluye en los gráficos debido a que la tendencia alrededor del punto le da consistencia, no se lo considera numéricamente válido. De todas formas está fuera del rango de operación del buque a esa escala.\\
$^{b}$~El valor $1+k = 1{.}622$ en 1:10 / $Fr = 0{.}160$ ($y^+ = 9{.}0$) se
aparta de la tendencia de su columna. Su ubicación en la región
``buffer'', por debajo del umbral de conmutación de la función de pared,
...
mayormente en la región ``buffer'' ($5 \lesssim y^+ \lesssim 30$). En
...
posición de la primera celda. No obstante, la región ``buffer'' constituye el régimen de menor fiabilidad para el tratamiento de pared, por lo
...
puntos individuales de la región ``buffer'' se dispersen en torno a ella.
...
\begin{figure}[htpb]
    ...
    \begin{subfigure}[b]{0.6\textwidth}
        \centering
        \includegraphics[width=\textwidth, height=0.25\textheight, keepaspectratio]{images/tpa_CFD_DB_165.pdf}
        \caption{Calado T1}
        \label{fig:k_165}
    \end{subfigure}
    ...
    \begin{subfigure}[b]{0.6\textwidth}
        \centering
        \includegraphics[width=\textwidth, height=0.25\textheight, keepaspectratio]{images/tpa_CFD_DB_180.pdf}
        \caption{Calado T2}
        \label{fig:k_180}
    \end{subfigure}
    ...
    \begin{subfigure}[b]{0.6\textwidth}
        \centering
        \includegraphics[width=\textwidth, height=0.25\textheight, keepaspectratio]{images/tpa_CFD_DB_195.pdf}
        \caption{Calado T3}
        \label{fig:k_195}
    \end{subfigure}

    \caption{Factor de forma calculado mediante el método de doble cuerpo (DB) para el \textit{P1} en los tres calados analizados, en función de Froude.}
    \label{fig:tpa_DB_k_evolucion}
\end{figure}

\subsubsection{Validación con modelo de referencia \textit{KCS}}
...
sobre el modelo de referencia \textit{KCS} (escala 1:79). La verificación numérica 
correspondiente se resume en la Tabla~\ref{tab:CFD_KCS}.

\begin{table}[htbp]
\centering
\caption{Verificación numérica para el modelo \textit{KCS} a escala 1:79.}
\label{tab:CFD_KCS}
\small
\begin{tabular}{ccccc}
\toprule
...
A diferencia del \textit{P1}, el $y^+$ del \textit{KCS} abarca desde la subcapa viscosa 
($y^+ \approx 0{.}3$) hasta el borde superior de la capa logarítmica 
($y^+ \approx 31$), atravesando la región ``buffer''. Pese a ello, la 
curva de $(1+k)$ resulta suave y sin cambios abruptos de pendiente, y el GCI se 
...
resistencia viscosa total, el factor de forma queda dominado por $C_F$, cuya estimación resulta robusta 
frente a la posición de la primera celda gracias al tratamiento de pared 
combinado del modelo $k$--$\omega$ SST (subcapa viscosa--ley logarítmica), 
en contraste con el \textit{P1}, donde $C_{PV}$ es comparable a $C_F$ 
(Sección~\ref{sec:descomposicion_de_la_res_viscosa}) y, al depender de la 
distribución de presión asociada al desarrollo de la capa límite, resulta 
...
reportados en \citet{TERZIEV2021147}. Ambas 
curvas presentan la joroba característica en el rango de transición 
($Re \approx 10^6 - 3\times10^6$), pero desplazada entre sí, probablemente 
como consecuencia de la pequeña diferencia de escala entre los modelos 
(1:79 en este trabajo frente a 1:75 en \citet{TERZIEV2021147}). Fuera de ese 
rango, en cambio, ambas curvas convergen hacia el mismo valor asintótico, 
lo que indica una buena concordancia hacia altos números de Reynolds. Las 
líneas punteadas verticales indican, como en las figuras anteriores, el rango de Froude de aplicación del método de Prohaska ($Fr = 0{.}10$ y $Fr = 0{.}20$), 
correspondientes al modelo \textit{KCS} a escala 1:79 ensayado en el LabHiNO, mientras que la línea 
...
\begin{figure}[htpb]
    ...
    \caption{Comparación de $1+k$ numérico vs $Re$ para el \textit{KCS} 1:79 (escala LabHiNO) y 1:75 \citep{TERZIEV2021147}.}
    \label{fig:kcs_DB_prohaska}
\end{figure}

Finalmente la Figura~\ref{fig:completo_kcs_lit} confirma que los resultados 
siguen la tendencia asintótica global validada por múltiples laboratorios  \citep{TerzievDataSet}.

\begin{figure}[htpb]
    \centering
    \includegraphics[width=.6\columnwidth]{images/cfd_kcs_lit.pdf}
    \caption{Factor de forma $(1+k)$ del \textit{KCS} propio (CFD DB) frente a la base de datos \citep{TerzievDataSet}.}
...
\paragraph{Resistencia} Las Figuras~\ref{fig:resis_tpa_efd_cfd_165}, \ref{fig:resis_tpa_efd_cfd_180} 
y~\ref{fig:resis_tpa_efd_cfd_195} comparan la resistencia experimental y
...
\begin{figure}[htpb]
    \centering
    \includegraphics[width=0.6\textwidth]{images/efd_cfd_165.pdf}
    \caption{Resistencia al avance del \textit{P1} en función de $Fr$. Comparación EFD--CFD para el Calado 1 (0.165\,m).}
    \label{fig:resis_tpa_efd_cfd_165}
\end{figure}

\begin{figure}[htpb]
    \centering
    \includegraphics[width=0.6\textwidth]{images/efd_cfd_180.pdf}
    \caption{Resistencia al avance del \textit{P1} en función de $Fr$. Comparación EFD--CFD para el Calado 2 (0.180\,m).}
    \label{fig:resis_tpa_efd_cfd_180}
\end{figure}

\begin{figure}[htpb]
    \centering
    \includegraphics[width=0.6\textwidth]{images/efd_cfd_195.pdf}
    \caption{Resistencia al avance del \textit{P1} en función de $Fr$. Comparación EFD--CFD para el Calado 3 (0.195\,m).}
...
y CFD para cada calado, junto con los puntos 
correspondientes a cada enfoque. Ambas rectas representan la extrapolación 
lineal sobre el rango reducido de Froude en cada caso, cuya ordenada al origen determina 
el valor de $(1+k)$ adoptado. Se observa que mientras los puntos experimentales tienden a curvarse de forma 
...
introducido por la línea de correlación ITTC-57. En el caso particular de 
...
\caption{Diagrama de Prohaska del \textit{P1} comparando EFD y CFD. Calado 1. Las líneas verticales punteadas indican $Fr = 0{.}10$ y $Fr = 0{.}20$ para el \textit{P1}.}
...
\caption{Diagrama de Prohaska del \textit{P1} comparando EFD y CFD. Calado 2. Las líneas verticales punteadas indican $Fr = 0{.}10$ y $Fr = 0{.}20$ para el \textit{P1}.}
...
\caption{Diagrama de Prohaska del \textit{P1} comparando EFD y CFD. Calado 3. Las líneas verticales punteadas indican $Fr = 0{.}10$ y $Fr = 0{.}20$ para el \textit{P1}.}
\label{fig:prohaska_tpa_efd_cfd_195}
\end{figure}

Las Figuras~\ref{fig:efd_cfd_165_tpa_range}, \ref{fig:efd_cfd_180_tpa_range} 
y \ref{fig:efd_cfd_195_tpa_range} muestran, para cada calado, el valor de 
$(1+k)$ obtenido por Prohaska al variar el límite inferior del rango de 
Froude considerado en la regresión, tanto para EFD como para CFD. 
Se observa que, al restringir el rango a $0.13 < Fr < 0.18$ aproximadamente, 
la brecha entre ambos enfoques se reduce respecto de la observada a bajas 
velocidades, si bien la magnitud de esa reducción depende del calado: la 
coincidencia es prácticamente completa en el Calado~1, mientras que en los 
Calados~2 y~3 persiste una diferencia entre el valor experimental y el 
numérico. Este comportamiento indica que, dentro del rango adecuado de 
aplicación del método, el ensayo experimental y la simulación numérica con 
superficie libre predicen factores de forma del mismo orden para el 
\textit{P1}, aunque no necesariamente idénticos.

\begin{figure}[htpb]
    \centering
    \includegraphics[width=0.7\textwidth]{images/efd_cfd_form_factor_tpa_165_range}
    \caption{Variación del factor de forma $(1+k)$ del \textit{P1} para el 
    Calado~1, obtenido mediante el método de Prohaska a partir de EFD y CFD, 
    en función del límite inferior del rango de Froude considerado en el 
    ajuste.}
...
\begin{figure}[htpb]
    \centering
    \includegraphics[width=0.7\textwidth]{images/efd_cfd_form_factor_tpa_180_range}
    \caption{Variación del factor de forma $(1+k)$ del \textit{P1} para el 
    Calado~2, obtenido mediante el método de Prohaska a partir de EFD y CFD, 
    en función del límite inferior del rango de Froude considerado en el 
    ajuste.}
...
\begin{figure}[htpb]
    \centering
    \includegraphics[width=0.7\textwidth]{images/efd_cfd_form_factor_tpa_195_range}
    \caption{Variación del factor de forma $(1+k)$ del \textit{P1} para el 
    Calado~3, obtenido mediante el método de Prohaska a partir de EFD y CFD, 
    en función del límite inferior del rango de Froude considerado en el 
    ajuste.}
...
Sección~\ref{sec:doble_cuerpo}. Las Figuras~\ref{fig:tpa_proj_DB_k_escala20_165}, 
\ref{fig:tpa_proj_DB_k_escala20_180} y \ref{fig:tpa_proj_DB_k_escala20_195} 
superponen, para cada calado, el valor adoptado por Prohaska EFD (línea horizontal 
roja) con los valores de $(1+k)$ obtenidos mediante el método de doble cuerpo, en función 
...
operativo del buque, comportamiento que ya fue observado para el \textit{KCS} y que 
será analizado en profundidad en la Sección~\ref{sec:analisis_efectos_escala}.

\begin{figure}[htpb]
    \centering
    \includegraphics[width=0.6\textwidth]{images/tpa_CFD_DB_prohaska_165.pdf}
    \caption{Comparación entre el valor de $(1+k)$ adoptado por Prohaska EFD
    (línea horizontal roja) y los valores obtenidos mediante el método de 
    doble cuerpo (DB) en función de $Fr$ (puntos azules) para el \textit{P1}, 
    Calado~1.}
...
\begin{figure}[htpb]
    \centering
    \includegraphics[width=0.6\textwidth]{images/tpa_CFD_DB_prohaska_180.pdf}
    \caption{Comparación entre el valor de $(1+k)$ adoptado por Prohaska EFD
    (línea horizontal roja) y los valores obtenidos mediante el método de 
    doble cuerpo (DB) en función de $Fr$ (puntos azules) para el \textit{P1}, 
    Calado~2.}
...
\begin{figure}[htpb]
    \centering
    \includegraphics[width=0.6\textwidth]{images/tpa_CFD_DB_prohaska_195.pdf}
    \caption{Comparación entre el valor de $(1+k)$ adoptado por Prohaska EFD
    (línea horizontal roja) y los valores obtenidos mediante el método de 
    doble cuerpo (DB) en función de $Fr$ (puntos azules) para el \textit{P1}, 
    Calado~3.}
...
La Tabla~\ref{tab:form_factors_efd_cfd} reúne los valores de $(1+k)$ adoptados
por el método de Prohaska para los tres calados, obtenidos a partir de los
ensayos experimentales (EFD) y de las simulaciones numéricas (CFD). Se observa
una coincidencia estrecha en el Calado~1, mientras que en los Calados~2 y~3 el
valor numérico resulta superior al experimental. Esta dispersión es coherente
con la curvatura de signo opuesto observada en los diagramas de Prohaska de
ambos enfoques y con la mayor sensibilidad del ajuste numérico al tratamiento
de la transición, y refuerza la conveniencia de recurrir al método de doble
cuerpo como determinación independiente del factor de forma.

\begin{table}[H]
\centering
\caption{Factor de forma $(1+k)$ del \textit{P1} adoptado mediante el método de
Prohaska para los tres calados, a partir de los ensayos experimentales (EFD) y
de las simulaciones numéricas (CFD).}
\begin{tabular}{lccc}
\hline
\textbf{Enfoque} & \textbf{T1: 0.165 m} & \textbf{T2: 0.180 m} & \textbf{T3: 0.195 m} \\
\hline
EFD Prohaska $(1+k)$ & 1.235 & 1.195 & 1.193 \\
CFD Prohaska $(1+k)$ & 1.193 & 1.314 & 1.254 \\
\hline
\end{tabular}
\label{tab:form_factors_efd_cfd}
\end{table}
...
\begin{itemize}
    ...
    \item La concordancia entre EFD y CFD mejora al restringir el análisis a 
    rangos donde los supuestos físicos del método de Prohaska son razonablemente 
    válidos, si bien subsiste una diferencia dependiente del calado que motiva 
    recurrir al método de doble cuerpo como determinación independiente.
\end{itemize}
...
dicho comportamiento con el correspondiente al \textit{KCS}. A diferencia de las 
...
de buques y escalas distintas (\textit{KCS} a 1:79 y 1:75, y \textit{P1} a 1:20) en un 
...
frente a $Re$ obtenidos mediante CFD ``double body'' (DB) para el \textit{KCS} 
a escala 1:79, el \textit{KCS} a escala 1:75 de \citet{TERZIEV2021147} y el \textit{P1} 
...
Prohaska en el presente trabajo, para el \textit{KCS} a escala 1:79 y para el 
\textit{P1}.

\begin{figure}[htpb]
	\centering
	\includegraphics[width=.6\columnwidth]{images/comparacion_total_kcs_tpa.pdf}
	\caption{$(1+k)$ frente a $Re$ para el \textit{KCS} (CFD DB, escalas 1:79 y 1:75 
...
observado en el \textit{KCS}. Por consiguiente, y en contraste con el comportamiento 
mostrado por el \textit{KCS}, el valor de $(1+k)$ del \textit{P1} calculado mediante 
...
\begin{figure}[htpb]
	...
	\caption{$(1+k)$ frente a $Re$ para el \textit{KCS} y el \textit{P1}, en escala 
	logarítmica. Para el \textit{KCS} se muestran los resultados propios obtenidos en 
	el LabHiNO (CFD DB y Prohaska EFD, escala 1:79) junto con el conjunto de 
	datos recopilado en \citet{TerzievDataSet} (CFD DB y Prohaska, otras 
...
el \textit{KCS} en la literatura, y extender el análisis del \textit{P1} a un rango 
...
disminuye para el \textit{KCS} a valores altos de $Re$, mientras que para el 
...
el \textit{P1}, mientras que para el \textit{KCS} es cercana al 8\%. Además, el valor 
final de $(1+k)$ del \textit{P1} resulta casi un 50\% superior al del \textit{KCS}, 
...
esta tesis (\textit{P2} con $L/B = 3{.}23$ y \textit{P3} con $L/B = 2{.}85$). 
...
marcado contraste con la estabilización del \textit{KCS}. Esta consistencia 
...
\begin{figure}[htpb]
    ...
    \caption{Evolución de $(1+k)$ con el número de Reynolds para el \textit{KCS} 
...
K_M$ en función de $\lambda$) a varias escaladas calculadas para el \textit{P1} (1:20, 1:18, 1:16, 1:14, 1:12, 1:10, 1:7.5 1:5, 1:2.5 y 1:1), únicamente a $Fr = 0.20$, la pendiente resultante es de
...
Cabe mencionar que el origen de esta inaplicabilidad es geométrico. Las cuatro familias de
cascos utilizadas en \citet{gomezgarcia} para calibrar su correlación corresponden a cascos esbeltos convencionales, con relaciones $L_{PP}/B$ comprendidas entre $6{.}98$ y $9{.}07$. Ninguna se aproxima siquiera a la baja esbeltez del los pesqueros analizados ($L/B \approx 2{.}8$--$3{.}5$). La correlación se aplica, por lo tanto, fuera del dominio para el que fue derivada, en un régimen donde la resistencia por presión viscosa, menor en los cascos esbeltos de la muestra original, pasa a ser una fracción dominante de la resistencia viscosa total, como se demuestra en la
...
en función de la razón de escala $\lambda$, con la recta de regresión lineal ajustada a los puntos calculados por CFD de doble cuerpo (DB). Se incluye, para comparación, la recta de \citet{gomezgarcia}.}
\label{fig:gomez-garcia-nuestro}
\end{figure}

De manera similar, el método de \citet{keh-sik-min} (Sección~\ref{sec:factor_de_forma_antec}) 
podría ser aplicado exitosamente al \textit{KCS}, dado que se observa dicho factor de 
...
La Figura~\ref{descomposicion_fuerzas} muestra $C_T$, $C_F$ y $C_{PV}$ obtenidos a partir de la simulación CFD en configuración de doble cuerpo (DB) para el \textit{P1}, en función del número de Reynolds ($Re$). Asimismo, se incluye la línea de correlación ITTC-57 ($C_{F0}$) a efectos comparativos.
...
configuración de doble cuerpo (DB), junto con la línea ITTC-57 ($C_{F0}$), en 
...
Ecuación~\eqref{eq:descomposicion_1k}, esta disminución lenta de $C_{PV}$ en el numerador, combinada con la disminución más rápida de $C_{F0}$ en el 
denominador, genera el incremento sostenido de $(1+k)$ observado. 
...
para el \textit{P1} y para el \textit{KCS}. Se observa que, en todo el rango de 
Reynolds considerado, el valor de $C_{PV}$ del \textit{P1} es sustancialmente 
superior al del \textit{KCS}, y que dicha diferencia se mantiene prácticamente sin 
disminuir a medida que aumenta $Re$. Cualitativamente, mientras que en el 
\textit{KCS} $C_F$ domina ampliamente sobre $C_{PV}$ en todo el rango analizado, en el 
...
\caption{$C_{PV}$ y $C_F$ obtenidos mediante CFD en doble cuerpo (DB) para el 
\textit{P1} y para el \textit{KCS} 1:79 (LabHiNO).}
\label{cpv_friccion}
\end{figure}
...
Interpretaciones similares de doble mecanismo han sido reportadas en 
...
escala modelo (1:20, $Re \approx 10^6$) y escala buque (1:1, $Re \approx 10^8$), donde se aprecia el adelgazamiento de la capa límite con $Re$, contribuyendo al primer mecanismo. Por otro lado, la Figura~\ref{vortex} muestra iso-superficies de criterio Q que revelan el sistema de estructuras 
...
Sección~\ref{sec:analisis_efectos_escala}. En el \textit{KCS}, la contribución de 
...
significativamente con $Re$, podría explicar la falta de valor 
...
de modo que $(1+k)$ ``decrece'' con la escala
...
del factor de forma entre modelo y buque, cuantificado mediante el ``transom form factor'' (factor de forma de popa espejo) $k_{tr}>0$ \citep{korkmaz_wetted_transom}. Su
...
sensible de $C_{PV}$, lo que hace que $(1+k)$ ``crezca'' de manera
...
``recuperar'' el comportamiento de placa plana, en los pesqueros de
...
considerablemente menor que en cascos más hidrodinámicos, como el \textit{KCS} 
...
distintos números de Reynolds, sobre tres planos horizontales a profundidad constante $|z|/T = 0$, $0{.}5$ y $0{.}75$ respectivamente, es decir, en la línea de flotación, a medio calado sumergido y cerca del fondo. La coordenada 
...
en \citet{korkmaz_cfd_2021} se reportó que la geometría de la proa bulbo puede 
...
comportamiento del \textit{KCS}. Ambas observaciones motivan la pregunta de 
investigación que organiza esta sección: ``¿es la proa bulbo 
...
simulaciones bifásicas con superficie libre para cada configuración. Siguiendo 
la descomposición de la resistencia total presentada en la 
Ec.~\eqref{eq:CT_decomposicion_hughes} (Sección~\ref{sec:extrapolacion}),
%
\begin{equation}
    C_T = C_V + C_W = (1+k)\, C_{F0} + C_W,
\end{equation}
%
el coeficiente $C_V$ se calculó como $C_V = (1+k)\, C_{F0}$, empleando, para 
cada configuración, el factor de forma $k$ determinado mediante el método de 
Prohaska a partir de sus propias simulaciones CFD bifásicas: para el modelo 
original con bulbo, el valor validado contra el ensayo experimental 
(Sección~\ref{sec:det_exp_fac_form}). Para el modelo Ctrl+Z, el valor obtenido 
aplicando el mismo procedimiento a sus simulaciones, sin contar con un ensayo 
experimental propio de validación. Finalmente, el coeficiente de resistencia 
por olas se obtuvo como residuo:
%
\begin{equation}
    C_W = C_T - C_V.
\end{equation}
...
responde a la presencia del bulbo. La anomalía reportada en 
...
\begin{figure}[htpb]
	...
	($T = 0{.}165$~m). Las líneas verticales punteadas indican $Fr = 0{.}10$ y $Fr = 0{.}20$ para el \textit{P1} (ambas configuraciones, con y sin bulbo, comparten el mismo rango de Froude).}
...
\begin{figure}[htpb]
	...
	\caption{Diagrama de Prohaska comparando modelos con y sin bulbo 
	($T = 0{.}195$~m). Las líneas verticales punteadas indican $Fr = 0{.}10$ y $Fr = 0{.}20$ para el \textit{P1} (ambas configuraciones, con y sin bulbo, comparten el mismo rango de Froude).}
	\label{fig:prohaska_bulbo_195}
\end{figure}

\paragraph{Calado $\mathbf{T = 0{.}165}$~m} La Figura~\ref{fig:Ct_bulbo_165} muestra 
...
\paragraph{Calado $\mathbf{T = 0{.}195}$~m} Para este calado, la 
...
\begin{figure}[htpb]
    ...
    \caption{Factor de forma $(1+k)$ del \textit{P1} con y sin proa bulbo, obtenido mediante el método de doble cuerpo (DB), en 
...
El estudio de \cite{korkmaz_wetted_transom} analiza la influencia del flujo con ``popa mojada'' (``wetted-transom flow'') sobre la predicción de la resistencia total y la extrapolación ITTC--78.  
Cuando el espejo de la popa se encuentra sustancialmente sumergido, ya sea por operar a baja velocidad o por calados elevados, se forma una \textbf{zona de recirculación} detrás del espejo, análoga al flujo que ocurre sobre un ``backward-facing step'' o ``escalón hacia atrás''.  
...
\begin{equation}
tr_{\text{ratio}} = \frac{A_{tr}}{A_{\max}},
\end{equation}

\begin{equation}
tr_{\text{fullness}} = \frac{A_{tr}}{2\,y_{tr}\,T_{tr}},
\end{equation}
...
El número de Froude de la popa espejo $Fr_{tr}$ representa una medida del grado de ``seco o mojado'' del espejo:  
...
El método ``two-form-factor'' o 2--$k$ fue propuesto para corregir el déficit de resistencia viscosa generado por la recirculación en popas sumergidas.  
...
siendo $k_M$ el factor de forma obtenido para el modelo (por método de Prohaska o CFD doble cuerpo), y $k_{tr}$ un incremento asociado al flujo recirculante detrás del espejo, denominado ``transom form factor''.  
Este último se expresa como:

\begin{equation}
k_{tr} \simeq \frac{C_{tr,S}}{C_{FS0}},
\end{equation}

donde $C_{tr,S}$ representa la contribución viscosa adicional debida al vórtice de recirculación, y $C_{FS0}$ el coeficiente de fricción a escala buque obtenido de la línea de fricción \citep{korkmaz_wetted_transom}.

El valor de $k_{tr}$ puede estimarse empíricamente mediante correlaciones con parámetros geométricos del espejo (secciones 6.3 y 7 de \cite{korkmaz_wetted_transom}), o bien por CFD comparativo entre modelo y escala buque, ya que bajo la hipótesis del método la diferencia total entre escalas coincide con la corrección de popa mojada:
%
\begin{equation}
k_S - k_M \simeq k_{tr}.
\end{equation}

Este punto es clave, ya que en \citet{korkmaz_wetted_transom} se atribuye la diferencia entre el factor de forma en escala modelo y en escala buque, en los casos de popa espejo sumergida, principalmente al flujo recirculante detrás del espejo. Esta interpretación se enmarca en el trabajo más amplio del mismo autor \citep{korkmaz_cfd_2021, korkmaz_wetted_transom}, donde se demuestra que la dependencia del factor de forma con el número de Reynolds desaparece casi por completo al sustituir la línea ITTC-57 por líneas de fricción numéricas derivadas del mismo código CFD, al menos para cascos de esbeltez moderada como el \textit{KCS} y el KVLCC2. El efecto de la línea de correlación sobre el \textit{P1} se analiza en la Sección~\ref{sec:linea_de_friccion}. Para cascos de esa clase, donde $C_{PV}$ es comparativamente pequeño, este resultado es consistente con la hipótesis de Hughes: una vez eliminado el sesgo de la línea de referencia, el factor de forma resulta prácticamente independiente de la escala. En qué medida esta atribución se verifica para un casco de muy baja esbeltez es lo que se examina en la aplicación al \textit{P1}.
...
$tr_{\text{ratio}} > 0{.}025$. Para espejos poco sumergidos o parcialmente secos, el uso del método 2--$k$ no resulta necesario.
...
La geometría de popa del \textit{P1} presenta una inmersión del espejo y según los criterios establecidos en \citet{korkmaz_wetted_transom} debe evaluarse en condición estática ($Fr = 0$), ya que el método demanda los parámetros geométricos correspondientes al buque en reposo. La Tabla~\ref{tab:tpa_transom_params} resume los parámetros calculados para esa condición.

El valor de $k_{tr}$ se obtiene, cuando $tr_{\text{ratio}} > 0{.}025$, mediante la
correlación empírica propuesta en \citet{korkmaz_wetted_transom}:
%
\begin{align}\label{eq:ktr_korkmaz}
    k_{tr} = {}&\Big[-0{.}025 + tr_{\text{ratio}}\big(1{.}5 - 2{.}3\,tr_{\text{ratio}} - 0{.}07\,\mathrm{LCB}\big)\Big] \notag\\
    &\times \Big[-5{.}45 + \log_{10}(\overline{Re_M})\big(1{.}415+4{.}32\,tr_{\text{ratio}}\big)
    - \big(\log_{10}(\overline{Re_M})\big)^{2}\big(0{.}081+0{.}55\,tr_{\text{ratio}}\big)\Big],
\end{align}
%
donde $\overline{Re_M}$ es el número de Reynolds promedio en escala modelo y
LCB es el centro longitudinal de carena referido a $L_{PP}/2$, expresado como
porcentaje de $L_{PP}$. Si $tr_{\text{ratio}} \leq 0{.}025$ se adopta
$k_{tr} = 0$, y si el valor resultante de la Ec.~\eqref{eq:ktr_korkmaz} es
negativo se trunca también a $k_{tr} = 0$.
...
establecido en \citet{korkmaz_wetted_transom}, por lo que corresponde aplicar 
...
Este resultado permite cuantificar la contribución de la popa espejo sumergida al efecto de escala total y, al mismo tiempo, descartarla como causa principal: la diferencia predominante tiene su origen en la evolución de $C_{PV}$, analizado en la Sección~\ref{sec:descomposicion_de_la_res_viscosa}. 
...
\citep{ITTC_resistance} y la NFL propuesta en \citet{Korkmaz2019NumericalFL}.
...
física del flujo, en particular a la evolución de $C_{PV}$ con $Re$, como se analizó en la 
Sección~\ref{sec:descomposicion_de_la_res_viscosa}.
...
introducen un cierre empírico a través de la denominada ``viscosidad 
...
El modelo $k$--$\omega$ SST (``Shear Stress Transport'') fue utilizado 
...
(``freestream sensitivity''), lo que puede afectar la predicción del 
...
(``realizability''), evitando la generación de energía cinética negativa 
...
lo reportado en \citet{Karim}, donde se observó una secuencia análoga en la 
...
\textit{P1} ($1+k \approx 1.30$ en $Re \approx 10^6$) para la simulación en escala modelo. El modelo 
...
Como se estableció en la Sección~\ref{sec:descomposicion_de_la_res_viscosa}, 
la persistencia de $C_{PV}$ frente a la caída de $C_{F0}$ impide que 
$(1+k)$ alcance un valor asintótico en el \textit{P1}, a diferencia de lo 
observado en el \textit{KCS}. Al comparar el valor de $(1+k)$ resultante con 
...
expresiones a este tipo de casco. Como consecuencia, la mayoría de las 
fórmulas disponibles subestima sustancialmente el valor observado mediante 
CFD DB para escala buque; la excepción relativa es la correlación de 
\citet{molland2017ship} (Watanabe), que se aproxima más al resultado 
numérico aunque también lo subestima, en este caso en un $4\%$. Esta 
cercanía, sin embargo, no debe interpretarse como una validación de dicha 
correlación para cascos de baja esbeltez: al depender de $(L/B)^{-2}$, la 
expresión de Watanabe decrece muy rápidamente con $L/B$, de modo que su 
proximidad al valor del \textit{P1} responde a la esbeltez particular de 
este casco antes que a una calibración adecuada para este rango; un casco 
apenas más esbelto haría que la misma fórmula se aleje rápidamente del 
valor CFD DB, como cualquiera de las restantes. La Tabla~\ref{tab:empiricos} 
presenta las expresiones utilizadas y los valores obtenidos para el 
\textit{P1}, y la Figura~\ref{empiricos} ilustra la dispersión entre 
correlaciones y la falta de coincidencia general con los resultados 
...
Para determinar si esta inconsistencia es propia del \textit{P1} o refleja 
una limitación estructural de estas correlaciones frente a cascos de baja 
esbeltez, se repitió el cálculo empleando los parámetros geométricos reales 
del \textit{P2}, el \textit{P3} y el \textit{KCS}, comparando en cada caso el 
resultado de las cinco fórmulas contra el valor CFD DB a escala buque y el 
valor EFD a escala modelo obtenidos para ese casco. La Figura~\ref{fig:empiricos_4cascos} 
resume esta comparación.
 
\begin{figure}[htpb]
    \centering
    \includegraphics[width=.8\textwidth]{./images/empiricas_4cascos.pdf}
    \caption{Correlaciones empíricas de $(1+k)$ aplicadas al \textit{P1}, el 
    \textit{P2}, el \textit{P3} y el \textit{KCS}, comparadas con el valor CFD DB a 
    escala buque (guion horizontal) y el valor EFD a escala modelo (cruz) 
    de cada casco.}
    \label{fig:empiricos_4cascos}
\end{figure}
 
El resultado confirma que la falla de las correlaciones no es un caso 
aislado del \textit{P1}, sino que se agrava sistemáticamente al disminuir 
$L/B$: para el \textit{P2} ($L/B=3{.}22$) y el \textit{P3} ($L/B=2{.}83$), 
las cinco fórmulas subestiman el valor CFD DB a escala buque en una 
proporción aún mayor que para el \textit{P1}, con diferencias que van del 
$19\%$ al $42\%$ según la fórmula y el casco considerados. En cambio, para el \textit{KCS} ($L/B=7{.}14$), un casco de 
esbeltez convencional, las mismas correlaciones se aproximan razonablemente 
tanto al valor CFD DB como al EFD, consistente con el régimen de $L/B$ para 
el que fueron calibradas. Esta tendencia es además coherente con los valores 
EFD de los tres pesqueros, que también crecen de manera monótona al 
disminuir $L/B$ ($1{.}19$, $1{.}487$ y $1{.}70$ para el \textit{P1}, el 
\textit{P2} y el \textit{P3} respectivamente), lo que sugiere que el 
comportamiento observado responde a una característica geométrica 
sistemática de los cascos de baja esbeltez, y no a una particularidad del 
\textit{P1}.
...
a $L/B$ en cascos de buques. En \citet{hoerner1965drag} se derivó por primera vez un factor de forma para cuerpos de revolución a partir de 
...
obtenidos por CFD DB para el \textit{P1} ($L/B = 3.53$) y para el \textit{KCS} 
...
\begin{figure}[htpb]
    ...
    \textit{P1} y el \textit{KCS}. Datos de \citep{hoerner1965drag, 
    raymer2019aircraft, torenbeek2010synthesis, roskam1987airplane, 
    shevell1989fundamentals, TerzievDataSet}.}
    \label{aerodynamics}
\end{figure}

El valor del \textit{KCS} coincide con la curva de \citet{hoerner1965drag}, coherente 
...
Sección~\ref{sec:presentacion_modelos} junto con el \textit{KCS}, fueron 
...
\begin{comment}\begin{figure}[h!]
    ...
    \begin{subfigure}{.18\textwidth}
        ...
        \textbf{\textit{KCS} (L/B = 7.12)}\\[3em]
        \textbf{P1 1 (L/B = 3.53)}\\[3.5em]
        \textbf{P2 (L/B = 3.23)}\\[3.5em]
        \textbf{P3 (L/B = 2.85)}
        \caption*{ }
    \end{subfigure}
    ...
    \begin{subfigure}{.24\textwidth}
        ...
        \caption*{ }
    \end{subfigure}
    \caption{Modelos ensayados. Los cuatro cascos son presentados con la misma eslora para comparación directa de su esbeltez.}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=.9\textwidth]{./images/modelos_esbeltez.png}
    \caption{Modelos ensayados. Los cuatro cascos son presentados con la misma eslora para comparación directa de su esbeltez.}
    \label{fig:LB_comparisson}
\end{figure}
...
buque, en marcado contraste con el \textit{KCS}, que exhibe el incremento inicial 
...
\begin{figure}[h]
    ...
    \caption{Comparación entre correlaciones aerodinámicas de factor de forma y los valores $(1+k)$ obtenidos mediante CFD DB para los tres pesqueros y el \textit{KCS}, representados en función de $l/d = L/B$.}
    \label{fig:aero_comparison_full}
\end{figure}

El gráfico revela una tendencia clara:  
\begin{itemize}
    \item El valor correspondiente al \textit{KCS} se ubica en la región de altas esbelteces y coincide con la curva clásica de Hoerner, como era de esperar para un casco alargado.
...
\begin{table}[htbp]
\caption{Diferencias porcentuales en la potencia efectiva estimada respecto al método ITTC (Método 1), $\Delta P_{ES} = (P_{ES} - P_{ES,1})/P_{ES,1}\times 100$.}
...
A diferencia del método 3, el método 2 no modifica el $k$ empleado para 
descomponer el ensayo de modelo (sigue usando $k_M$ para obtener $C_W$); 
lo que cambia es el $k$ aplicado al reconstruir $C_{TS}$ a escala buque, 
que pasa de $k_M$ a $k_S$:
%
\begin{equation}
    C_{TS}^{(2)} - C_{TS}^{(1)} = (k_S - k_M)\,C_{FS0}.
\end{equation}
%
Esta diferencia está gobernada por el salto $(k_S-k_M)$, sustancialmente 
mayor que el salto $k_M$ que gobierna la sobreestimación del método 3, 
precisamente porque $k_S \gg k_M$ 
es el resultado central de esta tesis. Por esa razón el método 2 se aleja 
más del método 1 que el método 3: no porque perturbe el procedimiento de 
manera más drástica, sino porque el salto de factor de forma involucrado 
es en sí mismo mucho mayor.
...
La comparación con correlaciones empíricas desarrolladas para buques mercantes confirma la incapacidad de dichas formulaciones para reproducir los valores de $(1+k)$ obtenidos para el \textit{P1}, tanto a escala modelo como hacia escala buque. En contraste, las correlaciones aerodinámicas basadas en cuerpos de revolución muestran una correspondencia notable con los resultados CFD DB, ubicando al \textit{KCS} y a los pesqueros en regiones coherentes con su respectiva esbeltez geométrica.
 
El comportamiento descrito no es exclusivo del \textit{P1}: los tres 
pesqueros estudiados, \textit{P1}, \textit{P2} y \textit{P3}, pese a sus 
diferencias geométricas particulares, presentan un comportamiento 
hidrodinámico consistente, caracterizado por valores elevados y no 
asintóticos del factor de forma. Esta consistencia indica que la 
dependencia de $(1+k)$ con la relación de aspecto constituye un rasgo 
estructural de los cascos pesqueros de baja esbeltez, común a los tres 
casos analizados.
