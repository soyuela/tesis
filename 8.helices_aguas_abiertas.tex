...
\begin{table}[htpb]
\centering
\caption{Comparación de $K_T$ y $10K_Q$ entre el ensayo del LabHiNO ($Re_n = 3.20\times10^5$) y los polinomios de \citet{oosterveld1975}, ajustados a $Re_{c,0.75R} = 2 \times 10^6$. Las diferencias se expresan en porcentaje respecto de MARIN.}
...
modo que $K_Q$ varía poco (una leve alza de $+0.6$ a $+1.6\%$ a $J = 0.30$--$0.40$). 

Se emplea aquí el operador $\delta$ para denotar la diferencia entre el cálculo sensible a la transición y el totalmente turbulento a una misma escala y punto de funcionamiento, $\delta K_T = K_T^{\text{SST-LM}} - K_T^{\text{SST}}$, en contraposición a $\Delta$, que en el resto de la tesis denota el efecto de escala. Como
...
los dos $Re_c$ extremos de la campaña. Las diferencias en empuje se expresan en porcentaje del valor totalmente turbulento; las de par se dan en valor absoluto, ya que la cancelación entre $\delta K_{Qp}$ y $\delta K_{Qf}$ hace que $\delta K_Q$ sea de magnitud muy pequeña y su expresión relativa resulte poco representativa.}
...
se analizaron los coeficientes locales $C_{p}$ y $C_f$ (Ecuación~\ref{eq:cpn_cf_def})
...
de $C_{p}$ es prácticamente insensible al modelo de turbulencia --la dispersión entre
...
\centering
\scriptsize
\caption{Predicción de $K_{TS}$ mediante ITTC-78 aplicado independientemente a cada
...
\begin{figure}[htpb]
    ...
    \caption{Predicción de $K_{TS}$ a escala buque mediante el procedimiento ITTC-78, aplicado a los cinco puntos CFD Fully Turbulent de escala modelo (panel a) y a cada uno de los cinco puntos experimentales de distinto $Re_c$ de modelo (panel b), comparado con el CFD Fully Turbulent resuelto directamente a escala buque (rombos negros).}
